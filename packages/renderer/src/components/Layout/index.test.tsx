...
import { render, fireEvent, ByRoleOptions } from '@testing-library/react';
...
test('renders 2 columns with a resize handle', () => {
  ...
  const handle = getByRole('handle' as ByRoleOptions, { exact: true});
  expect(handle).toBeInTheDocument();

  const panel = getByRole('panel' as ByRoleOptions, { exact: true });
  expect(panel).toBeInTheDocument();

  const editor = getByRole('editor' as ByRoleOptions, { exact: true });
